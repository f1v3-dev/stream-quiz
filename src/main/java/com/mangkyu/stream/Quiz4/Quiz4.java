...
public class Quiz4 {
    ...
    public List<Trader> quiz3() {
        return transactions.stream()
                .filter(transaction -> Objects.equals(transaction.getTrader().getCity(), "Seoul"))
    ...
    public boolean quiz5() {
        return transactions.stream()
                .filter(t -> Objects.equals(t.getTrader().getCity(), "Busan"))
                .findFirst().isPresent();
    }
    ...
    public List<Integer> quiz6() {
        return transactions.stream()
                .filter(t -> Objects.equals(t.getTrader().getCity(), "Seoul"))
                .map(t -> t.getValue())
                .collect(Collectors.toList());
    }
